...
class Auto_Roles(commands.Cog):
    ...
    async def reaction_role(self, ctx, *, msg):
        if (await self.config.blockCheck(ctx)) or not (
            await self.config.on_checkk(ctx, "autoR", True)
        ):
            return
        ...
        if (
            ...
            if int(len(autoRole) + 1) > 10:
                await ctx.channel.send(
                    embed=discord.Embed(
                        description="You've reached reaction role limit. To add reaction roles remove some",
            ...
            await ctx.channel.send(
                embed=discord.Embed(
                    description=f"Message sent, now add some configurations for it",
                    colour=discord.Color.dark_blue(),
                ),
                delete_after=20,
            )
            await asyncio.create_task(
                self.config.prnt(
                    ctx,
                    f"[{(await self.config.get_time())}] {self.config.getNAME(ctx, 'Auto Role')} added new auto role message",
                )
            )
        else:
            await ctx.channel.send(
    ...
    async def add_reaction_role(self, ctx, messageID, role: discord.Role, emoji):
        if (await self.config.blockCheck(ctx)) or not (
            await self.config.on_checkk(ctx, "add_autoR", True)
        ):
            return
        ...
        if (
            ...
            try:
                ...
                await ctx.channel.send(
                    embed=discord.Embed(
                        description="Couldn't find that message",
                        colour=discord.Color.red(),
                    )
                )
            ...
            try:
                ...
                if int(len(autoRole) + 1) > 8:
                    await ctx.channel.send(
                        embed=discord.Embed(
                            description="You've reached reaction role limit. To add reactoin roles remove some",
                            colour=discord.Color.red(),
                        )
                    )
                    await self.config.on_checkk(ctx, "add_autoR", False)
                ...
                if check_:
                    await ctx.channel.send(
                        embed=discord.Embed(
                            description="This role or emoji is already configured",
                            colour=discord.Color.red(),
                        )
                    )
                    await self.config.on_checkk(ctx, "add_autoR", False)
            ...
            await ctx.channel.send(
                embed=discord.Embed(
                    description=f"React {emoji} to get {role.mention} ",
                    colour=discord.Color.dark_blue(),
                ),
                delete_after=10,
            )
            await asyncio.create_task(
                self.config.prnt(
                    ctx,
                    f"[{(await self.config.get_time())}] {self.config.getNAME(ctx, 'Auto Role')} added new auto role",
                )
            )
        else:
            await ctx.channel.send(
    ...
    async def remove_reaction_role(self, ctx, messageID, num=0):
        if (await self.config.blockCheck(ctx)) or not (
            await self.config.on_checkk(ctx, "rm_autoR", True)
        ):
            return
        ...
        if (
            ...
            try:
                ...
                await ctx.channel.send(
                    embed=discord.Embed(
                        description="Couldn't find that messsage in config file",
                        colour=discord.Color.red(),
                    )
                )
            ...
            if int(num) == -1:
                ...
                if int(num) > int(length):
                    await ctx.channel.send(
                        embed=discord.Embed(
                            description="Out of range", colour=discord.Color.red()
                        )
                    )
                    await self.config.on_checkk(ctx, "rm_autoR", False)
                    return
            ...
            await ctx.channel.send(
                embed=discord.Embed(
                    description="Succcessfully remove reaction role config",
                    colour=discord.Color.dark_blue(),
                )
            )
            await asyncio.create_task(
                self.config.prnt(
                    ctx,
                    f"[{(await self.config.get_time())}] {self.config.getNAME(ctx, 'Auto Role')} removed auto role",
                )
            )
    ...
    async def reaction_role_list(self, ctx, messageID="None"):
        if (await self.config.blockCheck(ctx)) or not (
            await self.config.on_checkk(ctx, "autoR_list", True)
        ):
            return
        ...
        if (
            ...
            if len(autoRole) < 1:
                await ctx.channel.send(
                    embed=discord.Embed(
                        description="You don't have any reaction role configured",
                        colour=discord.Color.red(),
                    )
                )
            ...
            if str(messageID) == "None":
                ...
                try:
                    ...
                    await ctx.channel.send(
                        embed=discord.Embed(
                            description="Couldn't find that message in config file",
                            colour=discord.Color.red(),
                        )
                    )
                    await self.config.on_checkk(ctx, "autoR_list", False)
            ...
            await asyncio.create_task(
                self.config.prnt(
                    ctx,
                    f"[{(await self.config.get_time())}] {self.config.getNAME(ctx, 'Auto Role')} checked auto roles",
                )
            )
